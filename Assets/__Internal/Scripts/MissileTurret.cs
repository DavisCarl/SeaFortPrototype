...
public class MissileTurret : MonoBehaviour
{
    ...
    public void Fire()
    {
        if (canFire)
        {
            index++;
            index %= muzzles.Count;
            for (int i = 0; i < shellCount; i++) { Missile m = Instantiate(projectile, muzzles[index].position, muzzles[index].rotation); m.target = target; }
            currentTime = 0;
            canFire = false;
        }
    }
    ...
    {
        return projectedLocation = (target.velocity.magnitude / projectile.velocity) * target.velocity + target.position;
    }
